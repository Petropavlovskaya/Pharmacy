package by.petropavlovskaja.pharmacy.controller;

public final class AttributeConstant {
    private AttributeConstant() {
    }

    public static final String ERROR_MSG = "errorMessage";
    public static final String SUCCESS_MSG = "successMessage";
    public static final String SUCCESS_MSG_CHECK = "successTextSet";

    public static final String CUSTOMER = "customer";
    public static final String ACCOUNT = "account";
    public static final String ACTIVE_CUSTOMERS = "activeCustomers";
    public static final String ACCOUNT_LOGIN = "accountLogin";
    public static final String ACCOUNT_ID = "accountId";
    public static final String ACCOUNT_ROLE = "accountRole";
    public static final String ACCOUNT_SURNAME = "accountSurname";
    public static final String ACCOUNT_NAME = "accountName";
    public static final String ACCOUNT_PATRONYMIC = "accountPatronymic";
    public static final String ACCOUNT_PHONE = "accountPhone";

    public static final String MEDICINE_IN_CART = "medicineInCart";
    public static final String CART = "cart";

    public static final String AVAILABLE_MEDICINE = "availableMedicine";
    public static final String EDIT_MEDICINE = "editMedicine";
    public static final String MEDICINE_LIST = "medicineList";

    public static final String RECIPE = "recipe";

    public static final String ORDERS = "orders";

    public static final String MIN_DATE = "minDate";
    public static final String MAX_DATE = "maxDate";
    public static final String CURRENT_PAGE = "currentPage";
    public static final String RECORDS_PER_PAGE = "recordsPerPage";
    public static final String REQUEST_PAGE = "requestPage";
    public static final String NUMBER_OF_PAGES = "numOfPages";
    public static final String UPDATE_LANG = "updateLang";

    public static final String LOGIN = "login";
    public static final String PASSWORD = "password";
    public static final String OLD_PASSWORD = "oldPassword";
    public static final String NEW_PASSWORD = "newPassword";
    public static final String NEW_PASSWORD_CONFIRM = "newPasswordConfirm";
    public static final String PASSWORD_CONFIRM = "passwordConfirm";
}
