...
<h3 align="center">
    <c:if test="${sessionScope.get('currentPage') != 1}">
        <a href="${pageContext.request.contextPath}/${sessionScope.get("accountRole")}/medicine/list/${sessionScope.get('currentPage')-1}"><fmt:message key="label.pagination.previous"/></a>
...
<%--  maybe IF?          --%>
            <c:when test="${i eq 1}">
                <a href="${pageContext.request.contextPath}/${sessionScope.get("accountRole")}/medicine/list"><c:out value="${i}"/></a>
            </c:when>
            <c:otherwise>
                <a href="${pageContext.request.contextPath}/${sessionScope.get("accountRole")}/medicine/list/${i}"><c:out value="${i}"/></a>
    ...
    <c:if test="${sessionScope.get('currentPage') lt sessionScope.get('numOfPages')}">
        <a href="${pageContext.request.contextPath}/${sessionScope.get("accountRole")}/medicine/list/${sessionScope.get('currentPage')+1}"><fmt:message key="label.pagination.next"/></a>
